...
class LinkedList:
    ...
    def append(self, data):
        ...
        while current.next is not None: # None 이없다면 즉 뒤에 새 노드가 있다면
            current = current.next #다음 노드로 이동

        # ─── Level 3: 마지막 노드의 next 에 새 노드를 붙이기 ─────────
        # 위 반복문이 끝나면 current 가 곧 마지막 노드입니다.
        # TODO: current.next = new_node
        current.next = new_node #이동해서 끝나면 즉 마지막이라면 새로 만들어라(new_node)
    ...
    def print_list(self):
        ...
        #       current = current.next
        while current is not None:
            values.append(current.data) #리스트
            current = current.next
